...
async function loadData() {
  ...
  if (STATE.apiSettings.apiUrl === "") {
    ...
    if (localSettings) {
      STATE.project = JSON.parse(localSettings);
    }
...
function getTimelineMetrics() {
  ...
  // Bound week number between 1 and 13 (90 days is ~12.8 weeks, let's treat 13 as final)
...
function evaluateWeightLossSafety(weight, target) {
  ...
  if (kgPerWeek <= 0.5) {
    ...
    return {
      valid: false,
      class: "alert-danger",
      message: `❌ เป้าหมายนี้อันตรายเกินไป! การลดน้ำหนักมากกว่า 15% ของน้ำหนักตัว (${kg.toFixed(1)} กก.) ภายใน 90 วัน เสี่ยงต่อการโยโย่และกล้ามเนื้อสลายตัวสูง แนะนำให้ระบุเป้าหมายลดไม่เกิน 10% หรือประมาณ ${(wt * 0.1).toFixed(1)} กิโลกรัมก่อนในระยะแรก`
    };
  }
}
...
function handleTargetWeightEvaluation() {
  ...
  if (!isNaN(heightVal) && heightVal > 0) {
    ...
    if (bmi < 23.0) {
      ...
      feedbackDiv.innerHTML = `<svg viewBox="0 0 24 24" width="20" height="20" stroke="currentColor" fill="none" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg> <span>ขออภัยค่ะ โครงการนี้สำหรับผู้ที่มีค่า BMI 23.0 ขึ้นไปเท่านั้น (ค่า BMI ของท่านคือ ${bmi.toFixed(1)})</span>`;
...
async function handleRegistrationSubmit(e) {
  ...
  if (!employeeId || !firstName || !lastName || !department || !phone || isNaN(height) || isNaN(weight) || isNaN(targetWeightLoss) || !password) {
  ...
  // Validate BMI is 23.0 or above
  const bmi = weight / Math.pow(height / 100, 2);
  if (bmi < 23.0) {
    alert(`ขออภัยค่ะ โครงการนี้สำหรับผู้ที่มีดัชนีมวลกาย (BMI) ตั้งแต่ 23.0 ขึ้นไปเท่านั้นค่ะ (ค่า BMI ปัจจุบันของท่านคือ ${bmi.toFixed(1)})`);
    return;
  }
...
function setupWeeklyReportForm() {
  ...
  } else if (metrics.currentWeek >= 13) {
    allowSubmit = false;
    validationMessage = `ขณะนี้ถึงสัปดาห์สุดท้าย (สัปดาห์ที่ 13) แล้ว ผลการวัดสุดท้ายจะคัดกรองโดยแอดมินเท่านั้นเพื่อความโปร่งใส พนักงานไม่ต้องส่งข้อมูลรอบสุดท้ายเองค่ะ`;
  } else {
...
function renderLeaderboard() {
  ...
  STATE.registrations.forEach(user => {
    ...
    // LOCF Algorithm to extract Week 1 (Initial) and Latest
    // Find initial (Week 1)
...
function renderEmployeeDashboard() {
  ...
  document.getElementById("db-user-dept").innerText = user.Department;
  ...
  const maxWeeksToProcess = Math.min(13, Math.max(1, timeline.currentWeek));
...
function handleComparisonWeekChange() {
  ...
  const heightMeters = parseFloat(user.Height) / 100;
  const timeline = getTimelineMetrics();
  const maxWeeks = Math.min(13, Math.max(1, timeline.currentWeek));
...
async function handleSaveSettings(e) {
  ...
  if (STATE.apiSettings.isMockMode) {
    ...
    saveMockState();
    toggleLoader(false);
    alert("อัปเดตวันเริ่มโครงการจำลองสำเร็จ");
    renderAdminPanel();
  } else {
    try {
      const res = await callApiJSONP("updateSettingsJSONP", payload);
      toggleLoader(false);
      if (res.success) {
        alert("บันทึกวันเริ่มโครงการสำเร็จ");
        await loadData();
        renderAdminPanel();
      } else {
...
function renderAdminOverviewMetrics() {
  ...
  document.getElementById("admin-stat-total").innerText = totalRegistered;
...
function renderAdminParticipantsTable() {
  ...
  STATE.registrations.forEach(user => {
    ...
    if (initial) {
      ...
      const initialBMI = parseFloat(initial.Weight) / (heightM * heightM);
      const latestBMI = latestWeight / (heightM * heightM);
      const diffPercent = ((initialBMI - latestBMI) / initialBMI) * 100;
...
function renderExecutiveReportsData() {
  ...
  // Bound max week to project duration (13 weeks)
  const lastWeekIdx = Math.min(13, maxWeekAvailable);
